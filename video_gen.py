...
def create_simple_text_clip(text, duration=5):
    """Create a simple text clip with maximum compatibility"""
    try:
        # Try the most basic text clip creation
        import moviepy
        print(f"MoviePy version: {moviepy.__version__}")
        
        # Method 1: Try with just text
        clip = TextClip(text)
        if hasattr(clip, 'set_duration'):
            clip = clip.set_duration(duration)
        return clip
    except Exception as e:
        print(f"Could not create text clip: {e}")
        return None
        ...
        for i, item in enumerate(script_with_images):
            ...
            if item['image']:
                ...
                if image_path:
                    ...
                    # For now, skip text overlays to get basic video working
                    print("Skipping text overlay for compatibility - creating video with images and audio only")
                    video_clip = img_clip
                    
                    # Set audio to the video clip BEFORE compositing
